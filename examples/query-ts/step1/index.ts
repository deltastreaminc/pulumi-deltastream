...
// Pageviews stream (source)
const pageviews = new deltastream.DeltaStreamObject("pageviewsStream", {
    database: db.name,
    namespace: nsName,
    store: kafkaStore.name,
    sql: pulumi.interpolate`CREATE STREAM PAGEVIEWS (viewtime BIGINT, userid VARCHAR, pageid VARCHAR) WITH ('topic'='${pageviewsTopic}','value.format'='json');`,
}, { provider });
